...
export const createTimetableViewService = (): TimetableViewService => {
  return {
    getDayRange: (timetable) => {
      const startDay = 0;
      const endDay = Math.max(...timetable.lecture_list.flatMap((l) => l.class_time_json).map((c) => c.day), 4) as Day;
      return [startDay, endDay];
    },
    getHourRange: (timetable) => {
      const times = timetable.lecture_list.flatMap((l) => l.class_time_json);
